 ...
 * Implements AIProvider interface for OpenAI's GPT API
 */
...
export class OpenAIProvider implements AIProvider {
  ...
  readonly displayName = PROVIDER_METADATA.openai.displayName;
  ...
  async rewritePrompt(
    ...
    try {
      ...
      const model = modelId || PROVIDER_METADATA.openai.defaultModel;
  ...
  async validateKey(apiKey: string): Promise<boolean> {
    ...
    try {
      ...
      console.error('[OpenAIProvider] Key validation failed:', error);
      return false;
    }
  }
  ...
  private parseResponse(responseText: string, originalPrompt: string): ProviderRewriteResult {
    ...
    try {
      ...
      if (/\[[^\]]*입력\]|\[[^\]]*설명\]|\[[^\]]*정보\]/.test(rewritten)) {
        console.warn('[OpenAIProvider] Response contains placeholders, cleaning');
  ...
  private handleError(error: unknown): ProviderRewriteResult {
    ...
    if (error instanceof OpenAI.APIError) {
      ...
      if (error.status === 500 || error.status === 503) {
        return { success: false, error: 'OpenAI API 서버에 일시적인 문제가 있습니다.' };
      }
