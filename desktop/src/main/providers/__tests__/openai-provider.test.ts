...
describe('OpenAIProvider', () => {
  ...
  describe('rewritePrompt', () => {
    it('should successfully rewrite prompt', async () => {
      mockCreate.mockResolvedValue({
        choices: [
          {
            message: {
              content: 'Please analyze and fix the bug in the authentication system.',
            },
          },
        ],
      });

      const result = await provider.rewritePrompt(mockRequest, 'sk-test123');
    ...
    it('should use custom model when provided', async () => {
      ...
      await provider.rewritePrompt(mockRequest, 'sk-test123', 'gpt-4-turbo');
    ...
    it('should include context in request', async () => {
      ...
      await provider.rewritePrompt(mockRequest, 'sk-test123');
  ...
  describe('validateKey', () => {
    it('should validate correct API key', async () => {
      ...
      const result = await provider.validateKey('sk-test123');
    ...
    it('should return false on API error', async () => {
      ...
      const result = await provider.validateKey('sk-invalid');
    ...
    it('should extract JSON from mixed content', async () => {
      mockCreate.mockResolvedValue({
        choices: [
          {
            message: {
              content: 'Here is the result: {"rewrittenPrompt": "Improved", "explanation": "Better"} Done!',
            },
          },
        ],
      });
    ...
    it('should handle malformed JSON gracefully', async () => {
      mockCreate.mockResolvedValue({
        choices: [
          {
            message: {
              content: 'Some text {invalid json} more text',
            },
          },
        ],
      });
